...
def filter_worker_containers(containers: List[str], excluded_patterns: List[str] = None) -> List[str]:
    ...
    excluded_patterns = list(set(excluded_patterns + ['rabbitmq', "manager_2"]))
...
def chaos_monkey_loop(interval: int = 5, excluded_patterns: List[str] = None):
    ...
    try:
        ...
        print(f"Selected victim: {workers}")
        
        kill_container(workers)
...
def main():
    ...
    chaos_monkey_loop( excluded_patterns=args.exclude)
