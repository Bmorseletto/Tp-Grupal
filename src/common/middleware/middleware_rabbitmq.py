...
class MessageMiddlewareExchangeRabbitMQ(MessageMiddlewareExchange):
    """RabbitMQ implementation of exchange-based communication using a direct exchange.

    Declares a durable direct exchange and binds each routing key to a queue
    named after the routing key itself. Publishing sends the message to all
    bound routing keys; consuming listens on exactly one routing key.
    Messages are persisted and consumed with manual ack/nack and fair
    dispatch (prefetch_count=1). Thread-safe sending via a lock.
    """
